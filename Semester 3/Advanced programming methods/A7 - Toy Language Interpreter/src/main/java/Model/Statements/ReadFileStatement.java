...
public class ReadFileStatement implements iStatement {
    ...
    public ProgramState execute(ProgramState state) throws ExpressionException, DataStructureException, StatementException {
        ...
        iHeap memoryHeap = state.getHeap();
        iHashMap<String, BufferedReader> filesTable = state.getFilesHashmap();
        iValue variableVal;
        iValue expVal = __exp.eval(variablesTable, memoryHeap);
    ...
    public iHashMap<String, iType> typeCheck(iHashMap<String, iType> typeEnvironment) throws StatementException, DataStructureException, ExpressionException {
        if(!__exp.typeCheck(typeEnvironment).equals(new StringType()))
            throw new StatementException("Reading a file requires a string which represents the path of the file.");
        if(typeEnvironment.find(__var).equals(new IntType()))
            throw new StatementException("Reading a file requires an int variable to be read");
