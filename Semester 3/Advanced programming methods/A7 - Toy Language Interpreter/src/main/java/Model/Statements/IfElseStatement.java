...
public class IfElseStatement implements iStatement {
    ...
    public ProgramState execute(ProgramState state) throws ExpressionException, DataStructureException, StatementException {
        iValue expressionResult = __expression.eval(state.getHashmap(), state.getHeap());
    ...
    public iHashMap<String, iType> typeCheck(iHashMap<String, iType> typeEnvironment) throws StatementException, DataStructureException, ExpressionException {
        ...
        if(!typeExpression.equals(new BoolType()))
            throw new StatementException("If statement condition is not of type bool");

        __first.typeCheck(typeEnvironment.copy());
        __second.typeCheck(typeEnvironment.copy());
